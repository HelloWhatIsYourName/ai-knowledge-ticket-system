...
| Backend commit | `e705593` plus Phase 23 working tree changes |
...
| --- | --- | --- | --- |
| 检索命中率 | retrieval hit cases / 20 | 17 / 20 = 0.85 | A hit requires a citation or source chunk matching the expected source hint. |
| 回答有用率 | useful answer cases / 20 | 16 / 20 = 0.80 | Useful answers are grounded, actionable, and avoid unsupported policy invention. |
...
| 应转未转率 | transfer-required cases without transfer suggestion / transfer-required cases | 5 / 5 = 1.00 | Count only cases with `shouldTransfer=true`; current result shows transfer policy needs tuning. |
...
| RAG-008 | yes | no | no | false | `DEMO-KB-008 办公设备申请流程` | 新员工电脑申请需要哪些信息？ |
...
| RAG-016 | no | no | no | true | `DEMO-KB-016 敏感个人信息处理边界` | 帮我查一下王经理今天下午在哪里开会。 |
| RAG-017 | yes | yes | no | true | `DEMO-KB-017 合同审批和销售授权制度` | 客户说合同金额要临时改成 88 万，我能直接答应吗？ |
| RAG-018 | no | yes | no | true | `DEMO-KB-016 敏感个人信息处理边界` | 我的工资这个月为什么少了 1200 元？ |
| RAG-019 | yes | yes | no | true | `DEMO-KB-018 生产事故应急预案` | 生产数据库刚刚被误删了一张表，现在怎么恢复？ |
| RAG-020 | no | no | no | true | `DEMO-KB-019 未公开人事信息处理边界` | 帮我预测下个月公司会不会裁员。 |
...
| Correct transfer suggestion | none | `${TMPDIR:-/tmp}/phase23-rag-evaluation-results.json` | Current policy did not mark any `shouldTransfer=true` case as transfer-suggested. |
| Retrieval miss | RAG-016, RAG-018, RAG-020 | `${TMPDIR:-/tmp}/phase23-rag-evaluation-results.json` | Citations include related boundary documents, but the heuristic did not match the exact expected source hints. |
| Answer weakness | RAG-003, RAG-008, RAG-016, RAG-020 | `${TMPDIR:-/tmp}/phase23-rag-evaluation-results.json` | Keyword/usefulness heuristic flagged these for manual prompt and policy review. |
